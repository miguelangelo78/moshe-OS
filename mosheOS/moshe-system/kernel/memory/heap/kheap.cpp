// kheap.cpp -- Kernel heap functions, also provides
...
static int find_smallest_hole(uint32_t size, uint32_t page_align, heap_t *heap) {
	...
	if (iterator == heap->index.size) return -1; // We got to the end and didn't find anything.
...
void *alloc(uint32_t size, uint8_t page_align, heap_t *heap) {
	...
	if (page_align && orig_hole_pos & 0xFFFFF000) {
		uint32_t new_location = orig_hole_pos + PAGE_SIZE /* page size */ - (orig_hole_pos & 0xFFF) - sizeof(header_t);
